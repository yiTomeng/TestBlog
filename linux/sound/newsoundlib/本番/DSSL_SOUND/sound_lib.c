...
#define VOLUME_MAX			100												//音量最大値
#define VOLUME_MIN			0												//音量最小値
...
static void check_vol(long *vol);															//音量のチェック
static int set_volumn(const char *mixer_name, const long left_vol, const long right_vol);	//音声ボリュウムの設定
 ...
 * HISTORY      :
 * ID-----------DATE--------NAME----------NOTE --------------------------------
 * [V01.00.00]  2015.02.18  KOCHI.晏      西JR BUS
 *************************************************************************EOH*/
 ...
 * FUNCTION     : 音声ボリュウムを設定
 * VERSION      : 01.00.00
 * IN/OUT       : (i/ ) mixer_name: ミキサーの一つ項目名
 				: (i/ ) left_vol: 左ボリュウム
 				: (i/ ) right_vol: 右ボリュウム
 * RETURN       : 0:OK >0:NG　
 * REMARKS      : 備考
 ...
 * [V01.00.00]  2015.02.18  KOCHI.晏      西JR BUS
 *************************************************************************EOH*/
int set_volumn(const char *mixer_name, const long left_vol, const long right_vol)
{
	long pmin, pmax;
	float f_multi;
	int err;
	...
	//現在の1単位は実際のボリュウムに変換
	f_multi = (VOLUME_MAX - VOLUME_MIN) / (float)(pmax - pmin);
	
	//左ボリュウムを取得
	check_vol(&set_vol_left);
...
int play_file(snd_pcm_t *handler, const char *file_name, st_vol vol, int refresh_cnt)
{
	...
	//Master音声ボリュウムを設定
	res = set_volumn(MIXER_MASTER_NAME, vol.left_vol, vol.right_vol);
	if (ERR_ZERO != res)
	{
		return res;
	}
	...
	//PCM音声ボリュウムを設定
	res = set_volumn(MIXER_PCM_NAME, vol.left_vol, vol.right_vol);
	if (ERR_ZERO != res)
	{
		return res;
	}
